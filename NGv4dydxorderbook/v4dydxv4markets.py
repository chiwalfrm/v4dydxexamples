...
from v4dydxv4marketsclient import process_message
        ...
        def run(self):
                ...
                while True:
                        ...
                        if msg is None:
                                ...
                                mycursor = conn.execute("UPDATE v4mserver SET messageid = "+str(json.loads(msg)['message_id'])+" WHERE market1 = 'current';")
                                conn.commit()
...
def main():
        global pool
        mycursor = conn.execute("DELETE FROM v4mserver;")
        conn.commit()
        mycursor = conn.execute("INSERT INTO v4mserver VALUES ('current', -1);")
        conn.commit()
